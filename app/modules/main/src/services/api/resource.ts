...
export default class ResourceApi {
  ...
  async postById<T = unknown>(id: string | number, payload: any, config?: AxiosRequestConfig) {
  ...
  async deleteByParameters(payload: any, config?: AxiosRequestConfig) {
    const response = await this.api.apisauce.delete<BasicResponse>(this.endpoint, payload, config);
    return this.api.transformResponse(response);
  }

  async patch<T = unknown>(payload: any, config?: AxiosRequestConfig) {
